...
async def run_agent(
    ...
    try:
        agent, activity = await service.run_agent(
            client, body.agentId, user_id=user.id, role=user.role
        )
...
async def create_report(
    ...
    report, activity = await service.create_report(
        client, body.period, user_id=user.id, role=user.role
    )
...
async def preview_action(
    ...
    recommendation = await service.get_recommendation(
        client, body.recommendationId, user_id=user.id, role=user.role
    )
