"""End-to-end checks for the explicit public/user/business/admin boundaries."""

from tests.conftest import TEST_ADMIN_ID, TEST_BUSINESS_ID, TEST_USER_ID, factory_token
...
def _install(app):
    fake = FakeSupabase(rows={
        ...
        "vendors": [
            {"id": "vendor-a", "owner_id": TEST_BUSINESS_ID, "status": "active"},
            {"id": "vendor-b", "owner_id": "other-business", "status": "active"},
        ],
        "services": [
...
def test_exact_role_routes(client, app):
    _install(app)
    ...
    assert client.get("/api/v1/admin/metrics", headers=_auth(TEST_BUSINESS_ID)).status_code == 403
    assert client.get("/api/v1/admin/metrics", headers=_auth(TEST_ADMIN_ID)).status_code == 200


def test_business_lists_only_owned_resources(client, app):
    _install(app)
    response = client.get("/api/v1/business/services", headers=_auth(TEST_BUSINESS_ID))
    assert response.status_code == 200
    assert [row["id"] for row in response.json()] == ["service-a"]


def test_business_cannot_modify_another_business_resource(client, app):
    fake = _install(app)
    allowed = client.put(
        "/api/v1/business/services/service-a/status",
        json={"status": "archived"},
        headers=_auth(TEST_BUSINESS_ID),
    )
    denied = client.put(
